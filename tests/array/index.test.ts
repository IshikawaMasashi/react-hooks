...
import { useNumber } from '../../src/array/useNumber';
import { useArray } from '../../src/array/useArray';
import { useBoolean } from '../../src/array/useBoolean';
import { useInput } from '../../src/array/useInput';
import { useBindToInput } from '../../src/array/useBindToInput';
import { useSetState } from '../../src/array/useSetState';
import { useMap } from '../../src/array/useMap';
...
describe('useNumber array', () => {
  ...
  it('should increase value by predefined step with respect to upperLimit', () => {
    ...
    const { result } = renderHook(() =>
      useNumber(5, { step: 3, upperLimit: 7 })
    );
    const [, actions] = result.current;
    // when
    act(() => actions.increase());
  ...
  it('should decrease value by predefined step with respect to lowerLimit', () => {
    // given
    const { result } = renderHook(() =>
      useNumber(5, { step: 3, lowerLimit: 3 })
    );
    const [, actions] = result.current;
    // when
    act(() => actions.decrease());
...
describe('useSetState array', () => {
  it('should change and merge state', () => {
    ...
    const { result } = renderHook(() =>
      useSetState<State>({ field: 1, field2: 2 })
    );
  ...
  describe('hooks optimizations', () => {
    it('should keep actions reference equality after value change', () => {
      ...
      const [
        ,
        originalSetStateReference,
        originalResetStateReference,
      ] = result.current;
